#define rmp 7 //Replacing rmp with 3 every where in the program, so that we don't have to remember pin no.,as it is difficult to remember
#define rmn 6 //Same way as above
#define lmp 5 //Same way as above
#define lmn 4 //Same way as above
#define rop 8
#define ron 9
...
void loop()
{
  if(Serial.available()) //"Serial.available()" is a function which checks if Arduino is recieving information from Bluetooth module,if it is recieving signal then condition inside if() becomes true and then if()'s body is executed 
  {
    ...
    switch(key) //Now using switch() case we are trying to match recieved character with listed cases
    {
      ...
      case 'C':{rotleft();break;}
      case 'O':{rotright();break;}
      case 'S':{st();break;}  //case for stoping motion if any other key than listed above is pressed
    }
